...
describe('+ AuthController', () => {
  const mockUser = {
    ...
    password: 'zyxwvutsrqponmlkjihgfedcba',
  };
  let token = '';
  ...
  describe('+ GET: /connect', () => {
    ...
    it('+ Fails for a non-existent user', function (done) {
      this.timeout(5000);
      request.get('/connect')
        .auth('none@user.com', '7654300', { type: 'basic' })
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
    ...
    it('+ Fails with a valid email and wrong password', function (done) {
      this.timeout(5000);
      request.get('/connect')
        .auth(mockUser.email, '7654300', { type: 'basic' })
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
    ...
    it('+ Fails with an invalid email and valid password', function (done) {
      this.timeout(5000);
      request.get('/connect')
        .auth('hero@mmb.com', mockUser.password, { type: 'basic' })
        .expect(401)
        .end((err, res) => {
          if (err) {
            return done(err);
